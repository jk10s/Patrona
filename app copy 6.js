...
app.post('/registrar', (req, res) => {
    ...
    if (!nombre || !email || !password) {
        console.log('Error al registrar el usuario. Faltan los siguientes campos:',
          ...
          (!email ? 'email' : ''),
          (!password ? 'password' : ''));
        return res.send('Error al registrar el usuario. Datos incompletos.');
    }
    ...
    connection.query(query, values, async (error, results) => {
        if (error) {
            ...
            // Mostrar una alerta simple en lugar de SweetAlert
...
app.get('/ventas-pendientes', (req, res) => {
    ...
    connection.query(query, (error, results) => {
        if (error) {
            ...
            res.render('ventas-pendientes', { ventasPendientes: results });
        }
    });
});
